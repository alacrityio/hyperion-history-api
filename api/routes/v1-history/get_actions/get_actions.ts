...
async function fetchAbiHexAtBlockElastic(
  client,
  chain,
  contract_name,
  last_block,
...
async function getContractAtBlock(
  esClient,
  rpc,
  chain,
  accountName: string,
  ...
  let savedAbi, abi;
  savedAbi = await fetchAbiHexAtBlockElastic(
    esClient,
    chain,
    accountName,
    block_num,
...
async function getActions(fastify: FastifyInstance, request: FastifyRequest) {
  ...
  const esOpts = {
    ...
    from: from || 0,
    size:
      size > fastify.manager.config.api.limits.get_actions
        ? fastify.manager.config.api.limits.get_actions
        : size,
    body: {
      query: queryStruct,
      sort: {
