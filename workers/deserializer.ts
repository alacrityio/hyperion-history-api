...
function extractDeltaStruct(deltas) {
  ...
  for (const table_delta of deltas) {
    if (
      table_delta[0] === "table_delta_v0" ||
      table_delta[0] === "table_delta_v1"
    ) {
...
export default class MainDSWorker extends HyperionWorker {
      ...
      case "update_abi": {
        if (msg.abi) {
          if (msg.abi.abi_hex) {
            ...
            hLog(
              `Worker ${process.env.worker_id} updated the abi for ${msg.abi.account}`
            );
          }
        }
        break;
            ...
            if (trace[1] && trace[1].action_traces.length > 0) {
              ...
              try {
                if (trace[1].partial[1].signatures) {
                  ...
                  if (
                    trace[1].partial[1].prunable_data.prunable_data[1]
                      .signatures
                  ) {
                    signatures =
                      trace[1].partial[1].prunable_data.prunable_data[1]
              ...
              let filtered = false;
              if (
                this.conf.indexer.max_inline &&
                inline_count > this.conf.indexer.max_inline
              ) {
                trace[1].action_traces = trace[1].action_traces.slice(
                  0,
                  this.conf.indexer.max_inline
                );
                filtered = true;
  ...
  routeToPool(trace, headers) {
    ...
    if (trace["action_traces"][0] && trace["action_traces"][0].length === 2) {
      ...
      if (
        first_action.act.account === this.conf.settings.eosio_alias + ".null" &&
        first_action.act.name === "nonce"
      ) {
        if (
          trace["action_traces"][1] &&
          trace["action_traces"][1].length === 2
        ) {
    ...
    switch (this.conf.scaling.routing_mode) {
        ...
        if (this.dsPoolMap[_code]) {
          ...
          for (const w of workers) {
            if (typeof this.ds_pool_counters[_code] === "undefined") {
              ...
              if (
                this.ds_pool_counters[_code] === workers[workers.length - 1]
              ) {
      ...
      default: {
        hLog(
          `Invalid scaling.routing_mode option "${this.conf.scaling.routing_mode}". Exiting now!`
        );
        process.exit(1);
      }
    }
  ...
  async verifyLocalType(contract, type, block_num, field) {
    ...
    if (!abiStatus) {
      savedAbi = await this.fetchAbiHexAtBlockElastic(
        contract,
        block_num,
        false
      );
  ...
  async processContractRowNative(row, block) {
    ...
    const [_status, tableType, validFrom, validUntil] =
      await this.verifyLocalType(row["code"], row["table"], block, "table");
    if (_status) {
      let result;
      try {
  ...
  async getContractAtBlock(
    ...
    if (
      savedAbi === null ||
      (savedAbi.actions && !savedAbi.actions.includes(check_action))
    ) {
  ...
  async getTableType(code, table, block) {
    ...
    if (this_table) {
      ...
      if (this_table) {
        type = this_table.type;
        const initialTypes = Serialize.createInitialTypes();
        contract.types = Serialize.getTypesFromAbi(
          initialTypes,
          currentABI.abi
        );
      } else {
  ...
  async processContractRow(row, block, validFrom, validUntil) {
    const row_sb = this.createSerialBuffer(
      Serialize.hexToUint8Array(row["value"])
    );
    let error;
    try {
      const tableType: Type = await this.getTableType(
        row["code"],
        row["table"],
        block
      );
  ...
  deltaStructHandlers = {
    contract_row: async (payload, block_num, block_ts, row, block_id) => {
      ...
      if (
        this.conf.features.index_all_deltas ||
        payload.code === this.conf.settings.eosio_alias ||
        payload.table === "accounts"
      ) {
        ...
        if (jsonRow && (await this.processTableDelta(jsonRow))) {
          if (
            !this.conf.indexer.disable_indexing &&
            this.conf.features.index_deltas
          ) {
    ...
    account: async (account, block_num, block_ts) => {
      if (account["abi"] !== "") {
        try {
          ...
          const initialTypes = Serialize.createInitialTypes();
          const abiDefTypes: Type = Serialize.getTypesFromAbi(
            initialTypes,
            AbiDefinitions
          ).get("abi_def");
          const abiObj = abiDefTypes.deserialize(
            this.createSerialBuffer(abiBin)
          );
          ...
          debugLog(
            `📝  New code for ${account["name"]} at block ${block_num} with ${abi_actions.length} actions`
          );
          ...
          debugLog(
            `[Worker ${process.env.worker_id}] read ${account["name"]} ABI at block ${block_num}`
          );
          const q = this.chain + ":index_abis:1";
          await this.preIndexingQueue.push({
            queue: q,
            content: bufferFromJson(new_abi_object),
          });

          // update locally cached abi
          if (process.env["live_mode"] === "true") {
            hLog("Abi changed during live mode, updating local version...");
            const abi_update_status = AbiEOS["load_abi_hex"](
              account["name"],
              abiHex
            );
          ...
          if (this.autoBlacklist.has(account["name"])) {
            hLog(
              `${account["name"]} ABI updated, clearing dynamic blacklist entries...`
            );
          ...
          hLog(
            `Failed to process ABI from ${account["name"]} at ${block_num}: ${e.message}`
          );
        }
      } else {
    ...
    permission_link: async (link, block_num, block_ts, row) => {
      if (
        !this.conf.indexer.abi_scan_mode &&
        this.conf.indexer.process_deltas
      ) {
    ...
    generated_transaction: async (
      generated_transaction: any,
      block_num,
      block_ts
    ) => {
      if (
        !this.conf.indexer.abi_scan_mode &&
        this.conf.indexer.process_deltas &&
        this.conf.features.deferred_trx
      ) {
        // check delta blacklist chain::code::table
        ...
        const unpackedTrx = this.api.deserializeTransaction(
          Buffer.from(generated_transaction.packed_trx, "hex")
        );
        for (const action of unpackedTrx.actions) {
          const act_data = await this.deserializeActionAtBlockNative(
            action,
            block_num
          );
          if (act_data) {
    ...
    resource_limits: async (resource_limits, block_num, block_ts) => {
      if (
        !this.conf.indexer.abi_scan_mode &&
        this.conf.indexer.process_deltas &&
        this.conf.features.resource_limits
      ) {
    ...
    resource_usage: async (resource_usage, block_num, block_ts) => {
      if (
        !this.conf.indexer.abi_scan_mode &&
        this.conf.indexer.process_deltas &&
        this.conf.features.resource_usage
      ) {
          ...
          for (const row of deltaStruct[key]) {
            ...
            if (!data) {
              try {
                ...
                hLog(
                  `Delta struct [${key}] deserialization error: ${e.message}`
                );
                hLog(row.data);
              }
            }
  ...
  deserializeNative(datatype: string, array: any): any {
    if (this.abi) {
      try {
        return AbiEOS[
          typeof array === "string" ? "hex_to_json" : "bin_to_json"
        ]("0", datatype, array);
      } catch (e) {
  ...
  async deserializeActionAtBlockNative(_action, block_num): Promise<any> {
    const [_status, actionType] = await this.verifyLocalType(
      _action.account,
      _action.name,
      block_num,
      "action"
    ...
    if (_status) {
      try {
        return AbiEOS.bin_to_json(
          _action.account,
          actionType,
          Buffer.from(_action.data, "hex")
        );
      } catch (e) {
  ...
  private async populateTableHandlers() {
    ...
    this.tableHandlers[EOSIO_ALIAS + ":voters"] = (delta) => {
      ...
      delta["@voters"]["last_vote_weight"] = parseFloat(
        delta.data["last_vote_weight"]
      );
      delete delta.data["last_vote_weight"];
      delta["@voters"]["proxied_vote_weight"] = parseFloat(
        delta.data["proxied_vote_weight"]
      );
    ...
    this.tableHandlers[EOSIO_ALIAS + ".msig:proposal"] = async (delta) => {
      ...
      const trx = this.api.deserializeTransaction(
        Serialize.hexToUint8Array(delta.data["packed_transaction"])
      );
      for (const action of trx.actions) {
        action["hex_data"] = action["data"];
        action["data"] = await this.deserializeActionAtBlockNative(
          action,
          delta.block_num
        );
      }
    ...
    this.tableHandlers[EOSIO_ALIAS + ".msig:approvals2"] = (delta) => {
      delta["@approvals"] = {
        proposal_name: delta["data"]["proposal_name"],
        requested_approvals: delta["data"]["requested_approvals"].map(
          (item) => {
            return {
              actor: item.level.actor,
              permission: item.level.permission,
              time: item.time,
            };
          }
        ),
    ...
    this.tableHandlers["*:accounts"] = (delta) => {
      if (!delta.data) {
        ...
        if (delta.value.length === 32) {
          try {
            debugLog(
              `Attempting forced deserialization for ${delta["code"]}::accounts`
            );
            ...
            console.log(e);
            hLog(
              `Forced accounts table deserialization failed on ${delta["code"]}`
            );
          }
        }
      }
