...
export abstract class HyperionWorker {
  ...
  async connectAMQP() {
    [this.ch, this.cch] = await this.manager.createAMQPChannels(
      (channels) => {
  ...
  loadAbiHex(contract, block_num, abi_hex) {
    ...
    let _status;
    if (
      this.failedAbiMap.has(contract) &&
      this.failedAbiMap.get(contract).has(block_num)
    ) {
      _status = false;
  ...
  async loadCurrentAbiHex(contract) {
    ...
    if (
      this.failedAbiMap.has(contract) &&
      this.failedAbiMap.get(contract).has(-1)
    ) {
      _status = false;
