...
export default class DSPoolWorker extends HyperionWorker {
  ...
  constructor() {
    ...
    if (this.conf.api.tx_cache_expiration_sec) {
      if (typeof this.conf.api.tx_cache_expiration_sec === "string") {
        this.txCacheExpiration = parseInt(
          this.conf.api.tx_cache_expiration_sec,
          10
        );
      } else {
      ...
      if (existsSync(dir)) {
        ...
        for (const abiFile of files) {
          try {
            ...
            hLog(
              `Custom ABI for ${code} from ${startingBlock} up to ${endingBlock}`
            );
  ...
  async verifyLocalType(contract, type, block_num, field) {
    ...
    if (!_status) {
      if (this.conf.settings.allow_custom_abi) {
        if (this.customAbiMap.has(contract)) {
          ...
          const matchingAbi = list.find((entry) => {
            return (
              entry.startingBlock < block_num && entry.endingBlock > block_num
            );
          });
          if (matchingAbi.abi) {
      ...
      if (!_status) {
        const savedAbi = await this.fetchAbiHexAtBlockElastic(
          contract,
          block_num,
          false,
          0
  ...
  async deserializeActionAtBlockNative(self, _action, block_num): Promise<any> {
    ...
    const [_status, actionType] = await self.verifyLocalType(
      _action.account,
      _action.name,
      block_num,
      "action"
    ...
    if (_status) {
      try {
        ...
        debugLog(
          `(abieos) ${_action.account}::${_action.name} @ ${block_num} >>> ${e.message}`
        );
      }
    }
  ...
  async getContractAtBlock(accountName, block_num, check_action) {
    if (this.contracts.has(accountName)) {
      ...
      if (
        (_sc["valid_until"] > block_num && block_num > _sc["valid_from"]) ||
        _sc["valid_until"] === -1
      ) {
    ...
    if (check_action) {
      if (actions.has(check_action)) {
        if (
          !this.failedAbiMap.has(accountName) ||
          !this.failedAbiMap.get(accountName).has(-1)
        ) {
          try {
  ...
  async deserializeActionAtBlock(action, block_num) {
    const contract = await this.getContractAtBlock(
      action.account,
      block_num,
      action.name
    );
    if (contract) {
      if (contract[0].actions.has(action.name)) {
        try {
          ...
          debugLog(
            `(eosjs) ${action.account}::${action.name} @ ${block_num} >>> ${e.message}`
          );
          return null;
        }
      } else {
  ...
  async processTraces(transaction_trace, extra) {
    ...
    if (transaction_trace.status === 0) {
      ...
      if (this.mLoader.parser.flatten) {
        ...
        action_traces = await this.mLoader.parser.flattenInlineActions(
          action_traces,
          0,
          trace_counters,
          0
        ...
        action_traces.sort((a, b) => {
          return (
            a[1].receipt[1].global_sequence - b[1].receipt[1].global_sequence
          );
        });
      }
      ...
      for (const action_trace of action_traces) {
        if (action_trace[0].startsWith("action_trace_")) {
          const ds_status = await this.mLoader.parser.parseAction(
            this,
            ts,
            action_trace[1],
            trx_data,
      ...
      if (this.ioRedisClient && !this.conf.api.disable_tx_cache) {
        try {
          ...
          await this.ioRedisClient.expire(
            "trx_" + trx_data.trx_id,
            this.txCacheExpiration
          );
        } catch (e) {
          hLog(e);
        }
      ...
      this.monitoringLoop = setInterval(() => {
        if (this.totalHits > 0) {
          ...
          if (
            ...
            !this.conf.indexer.live_reader
          ) {
            this.noActionCounter++;
            if (this.noActionCounter > 60) {
              debugLog(
                `No actions processed for ${this.noActionCounter} seconds!`
              );
            }
          }
        }
  ...
  async run(): Promise<void> {
    debugLog(
      `Standalone deserializer launched with id: ${process.env.local_id}`
    );
  }
}
