...
export default class StateReader extends HyperionWorker {
  private readonly isLiveReader =
    process.env["worker_role"] === "continuous_reader";
  ...
  private range_size =
    parseInt(process.env.last_block) - parseInt(process.env.first_block);
  ...
  constructor() {
    ...
    if (this.isLiveReader) {
      this.local_block_num =
        parseInt(process.env.worker_last_processed_block, 10) - 1;
    }
  ...
  recursiveDistribute(data, channel, cb) {
    if (data.length > 0) {
      const q = this.isLiveReader
        ? this.chain + ":live_blocks"
        : this.chain + ":blocks:" + this.currentIdx;
      ...
      if (this.qStatusMap[q] === true) {
        if (this.cch_ready) {
          ...
          if (!this.isLiveReader) {
            ...
            debugLog(
              `Block Number: ${d.num} - Range progress: ${this.local_distributed_count}/${this.range_size}`
            );
  ...
  private signalReaderCompletion() {
    if (!this.completionSignaled) {
      ...
      debugLog(
        `Reader completion signal - ${this.range_size} - ${this.local_distributed_count}`
      );
      this.local_distributed_count = 0;
      this.completionMonitoring = setInterval(() => {
        ...
        if (pending === 0) {
          debugLog(
            `Reader completed - ${this.range_size} - ${this.local_distributed_count}`
          );
          clearInterval(this.completionMonitoring);
          process.send({
            event: "completed",
  ...
  private async processIncomingBlocks(block_array: any[]) {
    ...
    if (this.delay_active) {
      await new Promise((resolve) =>
        setTimeout(resolve, this.block_processing_delay)
      );
    }
    return true;
  }
  ...
  private async onMessage(data: Buffer) {
    ...
    if (!this.recovery) {
      ...
      if (result[0] === "get_status_result_v0") {
        ...
        const chain_state_begin_block =
          this.shipInitStatus["chain_state_begin_block"];
          ...
          switch (process.env["worker_role"]) {
            ...
            case "reader": {
              if (chain_state_begin_block > process.env.first_block) {
                ...
                hLog(
                  `First saved block is ahead of requested range! - Req: ${process.env.first_block} | First: ${chain_state_begin_block}`
                );
            ...
            case "continuous_reader": {
              this.requestBlocks(
                parseInt(process.env["worker_last_processed_block"], 10)
              );
              break;
            }
          }
        ...
        if (res["this_block"]) {
          ...
          if (this.isLiveReader) {
            ...
            if (blk_num !== this.local_block_num + 1) {
              hLog(
                `Expected: ${this.local_block_num + 1}, received: ${blk_num}`
              );
              try {
                ...
                hLog(
                  `Failed to handle fork during live reading! - Error: ${e.message}`
                );
              }
            } else {
            ...
            if (!this.receivedFirstBlock) {
              if (blk_num !== this.local_block_num + 1) {
                hLog(
                  `WARNING: First block received was #${blk_num}, but #${
                    this.local_block_num + 1
                  } was expected!`
                );
                hLog(
                  `Make sure the block.log file contains the requested range, check with "eosio-blocklog --smoke-test"`
                );
  ...
  private requestBlocks(start: number) {
    ...
    request.start_block_num = parseInt(
      first_block > 0 ? first_block.toString() : "1",
      10
    );
    ...
    if (this.ship.connected) {
      debugLog(
        `Reader ${process.env.worker_id} sending ${reqType} from: ${request.start_block_num} to: ${request.end_block_num}`
      );
  ...
  private send(req_data: (string | any)[]) {
    this.ship.send(
      serialize("request", req_data, this.txEnc, this.txDec, this.types)
    );
  }
  ...
  private async handleFork(data: any) {
    ...
    hLog(
      `Handling fork event: new block ${this_block["block_num"]} has id ${this_block["block_id"]}`
    );
  ...
  private startQueueWatcher() {
    setInterval(() => {
      if (this.isLiveReader) {
        this.manager
          .checkQueueSize(this.chain + ":live_blocks")
          .then((value) => {
